...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   load Simulated_Abundances
%   load back_grnd
% %  [nx,ny,p]=size(cat(3,Simulated_Abundances(:,:,1:p-1),Simulated_Abundances(:,:,end)));
%   s=reshape(cat(3,Simulated_Abundances(:,:,1:p-1),back_grnd(:,:,p-1)),N,p);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


s=s./repmat(norm_dim(s,1),size(s,1),1);
x = M * s' ; 
...
if isinf(SNR), n=zeros(L,N);Cn=zeros(L);      % no noise case
   ...
   if strcmp(noise_type,'Mixed')
      varianc = sum(x(:).^2)/10^((SNR)/10) /L/N ;
      % note that eta is 1/eta in equation (4) 
      quad_term = exp(-((1:L)-L/2).^2*eta^2/2);
      varianc_v = (.5*varianc)*L*quad_term/sum(quad_term);
      Cn1 = diag(varianc_v); 
      n1 = sqrtm(Cn1)*randn([L N]);
      %%
      factor = (.5*varianc)/mean(x(:)); 
      n2 = sqrt(factor) * sqrt(x) .* randn([L N]);
      Cn2 = diag(factor * mean(x,2)); %n*n'/N;
      Cn=Cn1+Cn2;
      n=n1+n2;
   end
end
